#!/bin/bash
################################################################################
# PHASE 11: Fix .htaccess Configuration
# Date: 2026-05-06
...
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "🔧 PHASE 11: Fixing .htaccess Configuration"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
cp /home/pim/public_html/.htaccess /home/pim/public_html/.htaccess.phase11.backup.$(date +%Y%m%d_%H%M%S) && {
    echo "✅ Backed up root .htaccess"
    ((FIXES_APPLIED++))
} || {
    echo "⚠️  Failed to backup root .htaccess"
    ((FIXES_FAILED++))
}

cp /home/pim/public_html/public/.htaccess /home/pim/public_html/public/.htaccess.phase11.backup.$(date +%Y%m%d_%H%M%S) && {
    echo "✅ Backed up public/.htaccess"
    ((FIXES_APPLIED++))
} || {
    echo "⚠️  Failed to backup public/.htaccess"
    ((FIXES_FAILED++))
}
...
if [ -f /home/pim/public_html/.htaccess ]; then
    ...
    echo "❌ Failed to update root .htaccess"
    ((FIXES_FAILED++))
fi
...
if [ -f /home/pim/public_html/public/.htaccess ]; then
    ...
    echo "❌ Failed to update public/.htaccess"
    ((FIXES_FAILED++))
fi
...
php bin/console cache:clear --env=prod --no-warmup 2>&1 | tail -5 && {
    ...
    echo "⚠️  Warning: Cache clear had issues (may be OK)"
    ((FIXES_APPLIED++))
}
...
php bin/console cache:warmup --env=prod 2>&1 | tail -5 && {
    ...
    echo "⚠️  Warning: Cache warmup had issues"
}
...
varnishadm 'ban req.url ~ /' 2>&1 && {
    ...
    echo "⚠️  Varnish cache clear may have failed (check manually)"
    ((FIXES_FAILED++))
}
...
sleep 2  # Give Apache time to fully reload
...
ROOT_STATUS=$(curl -sI http://localhost:8080/ | head -1)
echo "   Result: $ROOT_STATUS"

echo "▶ Test 2: Apache /user/login (port 8080)"
LOGIN_STATUS=$(curl -sI http://localhost:8080/user/login | head -1)
echo "   Result: $LOGIN_STATUS"

echo "▶ Test 3: Apache static asset (port 8080)"
ASSET_STATUS=$(curl -sI http://localhost:8080/bundles/pimui/images/logo.svg | head -1)
echo "   Result: $ASSET_STATUS"

echo "▶ Test 4: Varnish /user/login (port 80)"
VARNISH_STATUS=$(curl -sI http://localhost/user/login | head -1)
...
echo "4. Run: ./PHASE11_PLAYWRIGHT_TEST.sh (browser test)"
echo ""
echo "📁 Backup files created:"
ls -lh /home/pim/public_html/.htaccess.phase11.backup* 2>/dev/null | tail -2
ls -lh /home/pim/public_html/public/.htaccess.phase11.backup* 2>/dev/null | tail -2
echo ""
